...
class UserModelTests(TestCase):
    ...
    def test_create_superuser(self):
        User = get_user_model()
        admin_user = User.objects.create_superuser(username="adminuser", password="adminpass12#")
...
class AccountHistoryModelTests(TestCase):
    def setUp(self):
        ...
        self.account = Account.objects.create(owner=self.user, balance=Decimal("500.00"))

    def test_create_account_history(self):
        # create an account history entry and verify its properties
        history = AccountHistory.objects.create(account=self.account, amount=Decimal("100.00"))
    ...
    def test_account_history_auto_created_at(self):
        ...
        history = AccountHistory.objects.create(account=self.account, amount=Decimal("200.00"))
    ...
    def test_account_history_negative_amount(self):
        ...
        deposit = AccountHistory.objects.create(account=self.account, amount=Decimal("150.00"))
        withdrawal = AccountHistory.objects.create(account=self.account, amount=Decimal("-50.00"))
